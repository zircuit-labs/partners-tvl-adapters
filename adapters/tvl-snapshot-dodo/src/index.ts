...
import {
  AMM_TYPES,
  CHAINS, CSVRow,
  PositionData, PROTOCOLS, TokenBalance,
...
const OUTPUT_FILE = '../out/tvl-snapshot-dodo.csv';
...
const processPositionData = async (block: number): Promise<PositionData | null> => {
  try {
    ...
    try {
      classicPositions = await getUserClassicPositions(block, CHAINS.ZIRCUIT, PROTOCOLS.DODOEX, AMM_TYPES.DODO);
    } catch (error) {
